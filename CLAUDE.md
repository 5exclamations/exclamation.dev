...
## Бейдж для клиентских продуктов

`BADGE.md` — плашка «DEVELOPED BY ! EXCLAMATION», которую вставляют в подвал
клиентских сайтов и приложений. Собирается `npm run badge` из тех же шрифтов
и токенов, кладётся в `public/badge/` и уезжает на домен вместе с сайтом.
Готовые куски — `.claude/skills/exclamation-badge/SKILL.md`.

Руками SVG не перерисовывать и не перенабирать: буквы там контурами, любой
пересбор даёт другую плашку. Ссылка всегда на английскую главную.
