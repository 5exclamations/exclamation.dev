...
export const integrations: Record<Locale, ServiceCopy> = {
  ...
  ru: {
    ...
    intro: [
      ...
      'Дальше идемпотентность. Если запрос повторили, заказ не должен создаться дважды — за это отвечает уникальный ключ операции. Без него повторные попытки превращаются из страховки в источник дублей. Ставим по умолчанию, а не как опцию за отдельные деньги.',
      'И наблюдаемость.',
      'Интеграция ломается тихо: никто не получает ошибку, просто заказы перестают доходить, и замечают это через день по жалобе клиента. Поэтому в каждую связку встраивается мониторинг и уведомление: падение видим мы, а не вы. В кейсе Merkuri именно на этом держится обмен с таможенными базами — данные идут между участниками и подтверждаются.',
    ],
    scope: {
    ...
    pricing: {
      ...
      note: 'Схему потоков, этапы и точную смету отдаём за 3–5 дней после брифа, бесплатно.',
    },
    stack: {
      title: 'Стек',
    ...
    notFor: {
      ...
      items: [
        ...
        },
        {
          ...
          text: 'Интеграция фиксирует сегодняшнее правило в коде. Сначала стабилизируйте процесс.',
        },
        {
          cond: 'Объём данных маленький',
          text: 'Перенести пять заказов в день руками — это несколько минут. На таком объёме автоматизация не окупится годами. Посчитаем это на брифе и назовём цифру.',
        },
      ],
    ...
    faq: {
      ...
      items: [
        ...
        },
        {
          ...
          a: 'Обычно нет: интеграция строится поверх текущих систем, а замена нужна только там, где у системы нет ни одной точки входа — и это выясняется на аудите, не на середине разработки.',
        },
        {
          q: 'Партнёр поменял API — кто чинит?',
          a: 'В течение трёх месяцев гарантии — мы, бесплатно. Дальше это отдельная небольшая работа. Внешние API меняются раз-два в год, и предупреждаем об этом сразу. Поэтому в каждую связку ставится мониторинг. Поломку видим мы, а не вы через жалобу клиента.',
        },
      ],
    },
    cta: {
