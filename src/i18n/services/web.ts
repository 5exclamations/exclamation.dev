...
export const web: Record<Locale, ServiceCopy> = {
  ...
  ru: {
    ...
    intro: [
      'Начнём с честного: если задача — одна страница под одну акцию, соберите её на конструкторе за день. Tilda и подобные закрывают такие задачи лучше, чем что угодно написанное с нуля за те же деньги. Мы говорим это на брифе и не берём проекты, где ответ очевиден не в нашу пользу.',
      'Разница начинается там, где сайт перестаёт быть визиткой. Конструктор отдаёт тяжёлые страницы: чужие скрипты, невыключаемые библиотеки, картинки без нормальной оптимизации. На мобильном интернете это секунды ожидания, а первое впечатление складывается за три. Собранный статически сайт отдаётся с CDN, картинки конвертируются в avif и webp автоматически, шрифты лежат на своём домене — страница открывается меньше чем за секунду.',
      'Второе — многоязычность. В Баку она нужна почти всегда, и конструкторы делают её криво: один адрес, переключение текста на лету, поисковик видит одну страницу вместо трёх. Мы разносим языки по отдельным маршрутам с собственными адресами, мета-описаниями и разметкой `hreflang`. Три языка индексируются отдельно, и каждый находится по своему запросу.',
      'Третье — вы владеете результатом. Репозиторий в вашем аккаунте, домен на вас, абонплаты за конструктор нет. Если через два года вы захотите передать сайт другой команде, вы передаёте код и документацию, а не логин от чужой платформы.',
    ],
    scope: {
    ...
    pricing: {
      ...
      note: 'После брифа за 3–5 дней отдаём структуру, этапы и точную смету. Этап бесплатный и ни к чему не обязывает.',
    },
    stack: {
      title: 'Стек',
    ...
    timeline: {
      ...
      steps: [
        ...
        },
        {
          title: 'Дизайн',
          time: '1–2 недели',
          text: 'Макеты ключевых страниц, мобильные и десктопные. Остальные собираются из готовых компонентов.',
        },
        {
    ...
    notFor: {
      ...
      items: [
        ...
        },
        {
          ...
          text: 'Под разовую акцию или мероприятие конструктор собирается за день и делает свою работу. Писать это с нуля дорого и незачем.',
        },
        {
          cond: 'Нет текстов и фотографий и в ближайшее время не будет',
          text: 'Запустить сайт с пустыми блоками — значит запустить неработающий сайт. Дешевле дождаться контента и выпустить один раз.',
        },
        {
      ...
      close: 'Если один из пунктов про вас — всё равно напишите. Аудит и рекомендацию дадим бесплатно, и иногда вывод звучит как «переделывать сайт не нужно». Мы не стесняемся так отвечать.',
    },
      ...
      items: [
        {
          ...
          a: 'Для простой задачи — да, и мы говорим это прямо. Разница проявляется в скорости загрузки, в корректной многоязычности и в отсутствии ежемесячной абонплаты. На брифе считаем оба варианта.',
        },
        {
        ...
        },
        {
          ...
          a: 'В течение трёх месяцев гарантии ошибки и мелкие правки на нас. Новый раздел или функция — отдельный небольшой проект, стоимость называем заранее.',
        },
      ],
    },
    cta: {
