...
export const mobile: Record<Locale, ServiceCopy> = {
  ...
  ru: {
    ...
    intro: [
      ...
      'В большинстве проектов мы берём Flutter. Интерфейс рисуется собственным движком, поэтому приложение выглядит одинаково на обеих платформах, а сроки и последующая поддержка сокращаются примерно вдвое. Для витрины, личного кабинета, трекера, доставки, записи, лояльности этого достаточно с запасом.',
      'Натив нужен там, где приложение упирается в платформу: тяжёлая графика и анимация на 120 кадрах, серьёзная работа с камерой и обработкой видео, глубокая интеграция с системой — виджеты, часы, фоновая геолокация с экономией батареи. Если задача про это, кроссплатформа начнёт мешать примерно на третьем месяце, и честнее сказать об этом до старта, а не после.',
    ...
    pricing: {
      ...
      note: 'После брифа за 3–5 дней отдаём выбор платформы, этапы и точную смету. Этап бесплатный и ни к чему не обязывает.',
    },
    stack: {
      title: 'Стек',
    ...
    notFor: {
      ...
      items: [
        ...
        },
        {
          ...
          text: 'Каталог, цены, контакты — для этого достаточно сайта, и он ещё находится в поиске. Приложение в поиске не видно, его нужно продвигать отдельно.',
        },
        {
          cond: 'Аудитория не оправдывает две платформы',
          text: 'Если почти все пользователи на одной платформе, одновременный выпуск второй удваивает бюджет. Дешевле выпустить одну и посмотреть на цифры.',
        },
        {
          cond: 'Нет бюджета на продвижение',
          text: 'В магазине приложение само себя не находит. Установки требуют рекламы, существующей базы или офлайн-канала; без этого релиз пройдёт в тишине.',
        },
      ],
      close: 'Если один из пунктов про вас — всё равно напишите. Бесплатно скажем, что подходит: мобильный сайт, PWA или приложение. Иногда ответ — «приложение не нужно», и мы не стесняемся так отвечать.',
    },
      ...
      items: [
        {
          ...
          a: 'Для большинства задач Flutter: одна кодовая база собирается в оба магазина, сроки и поддержка сокращаются примерно вдвое. Натив — когда нужна тяжёлая графика, глубокая работа с камерой или системные интеграции. Выбор обосновываем письменно на этапе оценки.',
        },
        {
        ...
        },
        {
          ...
          a: 'Три месяца гарантии: падения и ошибки на нас. Новые функции — отдельные небольшие проекты. CI/CD настроен, поэтому новая версия уходит в магазин за несколько часов.',
        },
      ],
    },
    cta: {
