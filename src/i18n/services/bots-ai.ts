...
export const botsAi: Record<Locale, ServiceCopy> = {
  ...
  ru: {
    ...
    titleMain: 'снимают часть нагрузки, а не всю.',
    ...
    intro: [
      ...
      'LLM-ассистент понимает свободный текст и собирает ответ из ваших документов. Живее, но требует контроля. Контроль строится на RAG: модель не отвечает из памяти, а сначала находит нужный фрагмент в вашей базе и отвечает только по нему, со ссылкой на источник. Если фрагмент не нашёлся — ассистент говорит, что не знает, и передаёт диалог оператору.',
      'Второй элемент контроля — журнал. Все диалоги пишутся, слабые ответы видно, из них собирается следующая версия базы знаний. Первые месяцы бот улучшается в основном на этом, а не на смене модели.',
    ...
    pricing: {
      ...
      note: 'После брифа за 3–5 дней отдаём карту сценариев, этапы и точную смету. Этап бесплатный и ни к чему не обязывает.',
    },
    stack: {
      title: 'Стек',
      ...
      items: [
        {
          ...
          text: 'Если повторов нет, автоматизировать нечего. На таком объёме оператор и быстрее, и дешевле.',
        },
        {
    ...
    faq: {
      ...
      items: [
        ...
        },
        {
          ...
          a: 'Не выдумывает. Передаёт вопрос и контекст диалога оператору, а клиенту называет время ожидания. Такие случаи логируются — из них собирается следующая версия базы знаний.',
        },
        {
        ...
        },
        {
          ...
          a: 'Вы, через админку: добавляете документ, индекс обновляется сам. После крупных изменений качество ответов стоит перепроверить — как это делать, показываем на сдаче.',
        },
      ],
    },
    cta: {
