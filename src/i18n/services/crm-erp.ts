...
export const crmErp: Record<Locale, ServiceCopy> = {
  ...
  ru: {
    ...
    intro: [
      'Почти каждый разговор о CRM начинается с одного вопроса: брать готовое или писать своё. Честный ответ: чаще всего — готовое. amoCRM, Bitrix24 и подобные запускаются за неделю, стоят предсказуемых денег и закрывают воронку «лид — звонок — сделка» лучше, чем что угодно написанное с нуля за тот же срок.',
      'Своя система нужна там, где процесс не влезает в чужую модель. Признаки узнаваемые: нестандартные этапы сделки, своя логика расчёта цены, собственные формы документов, требования рынка, которых в коробке нет. Дальше начинается второй акт — плагины, интегратор, подписка, которая растёт с каждым новым пользователем. И половина процесса всё равно живёт в отдельной таблице.',
      'Разработка на заказ снимает этот выбор. Код с первого дня в вашем репозитории, данные в вашем аккаунте, помесячной платы за пользователей нет.',
      'Взамен — долгий старт и своя техническая команда.',
      'Ниже отдельный раздел о том, когда CRM вам не нужна вовсе. Читайте его первым.',
      'Два законченных цикла в этом направлении. CRM Portal — для агентств недвижимости: база объектов, Kanban-воронка, интеграция с WhatsApp и звонками; конверсия продаж выросла на 45%. FLEKS — ERP для психологического центра: балансы клиентов, списания за сеансы, автоматические PDF-чеки; оформление оплат ускорилось на 80%. Цифры считал клиент после запуска, не мы.',
    ],
    scope: {
    ...
    pricing: {
      ...
      lead: 'Прайс-листа у нас нет: за одним словом «CRM» стоит и двухнедельный проект, и полугодовой. Смету двигает вот это.',
      drivers: [
      ...
      note: 'Через 3–5 дней после брифа отдаём архитектуру, этапы и точную смету. Бесплатно и ни к чему не обязывает.',
    },
    stack: {
      title: 'Стек',
      note: 'Подбираем под проект. Ниже — то, с чем чаще всего работаем в CRM и ERP.',
      groups: [
        { name: 'Backend', items: ['Django', 'FastAPI', 'PostgreSQL', 'Celery'] },
        { name: 'Frontend', items: ['React', 'Next.js', 'TypeScript'] },
        { name: 'Integrations', items: ['REST API', 'Webhooks', 'WhatsApp API', '1C'] },
    ...
    notFor: {
      ...
      lead: 'Иногда CRM — это потраченные деньги и три месяца, которые ничего не изменили. Проверки, которые мы делаем до того, как взять проект.',
      items: [
        {
          cond: 'Меньше 20 сделок в месяц',
          text: 'На таком объёме аккуратная таблица и напоминания в календаре делают ту же работу, причём уже сегодня. Система начинает окупаться там, где вести вручную физически не выходит: когда карточки перестают помещаться в голову одного человека, когда забытый звонок стоит денег. Двадцать сделок — не тот случай.',
        },
        {
          cond: 'В команде один-два человека',
          text: 'Информации негде теряться. Коробка дешевле и быстрее.',
        },
        {
          cond: 'Процесс ещё не устоялся',
          text: 'Каждый месяц новая схема работы — система забетонирует вчерашнюю. Сначала стабилизируйте процесс, потом автоматизируйте: иначе заплатите дважды.',
        },
        {
          cond: 'Коробка ложится без переделок',
          text: 'Совпал процесс с моделью amoCRM или Bitrix24 — писать своё незачем. Увидим это на брифе, скажем сами и предложим настройку вместо разработки.',
        },
      ],
      close: 'Узнали себя в одном из пунктов — всё равно напишите. Бесплатно скажем, что полезнее сделать сейчас; иногда ответ — «пока ничего».',
    },
    ...
    faq: {
      ...
      items: [
        ...
        },
        {
          ...
          a: 'Нет. Перенос данных — отдельный этап: сначала пробный прогон, потом сверка, потом основной. Старая база всё это время работает параллельно. Выключаем её только тогда, когда вы сверили результат и согласны с ним.',
        },
        {
        ...
        },
        {
          ...
          a: 'Три месяца гарантии на нас: мониторинг и исправление ошибок. Дальше пакет поддержки или ваша команда — репозиторий и документация у вас с первого дня.',
        },
        {
          q: 'Как я буду видеть ход работы?',
          a: 'Спринты по две недели, в конце каждого рабочее демо, доступ к таск-трекеру у вас с первого дня.',
        },
      ],
    },
    cta: {
      title: 'Расскажите о задаче',
      text: 'Хватит короткого брифа: с чем работаете, сколько человек в команде, что теряется сейчас. Через 3–5 дней вернёмся с архитектурой и сметой.',
