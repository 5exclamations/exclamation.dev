...
export const ecommerce: Record<Locale, ServiceCopy> = {
  ...
  ru: {
    ...
    titleMain: 'на своей платформе, а не на чужой комиссии.',
    ...
    intro: [
      'Начнём с честного, как и на других страницах: если у вас типовой каталог и типовой checkout, берите готовое. Shopify, WooCommerce, OpenCart закрывают стандартный сценарий быстрее и дешевле, чем разработка с нуля. Мы говорим это на брифе и не берём проекты, где ответ очевиден не в нашу пользу.',
      'Своя платформа начинает окупаться на трёх вещах. Первая — нестандартная товарная модель: комплекты, конфигураторы, цены под клиента, единицы измерения, которые не ложатся в поле «количество». Вторая — правила скидок и лояльности, которые в коробке живут через плагины, а плагины конфликтуют друг с другом на каждом обновлении. Третья — синхронизация с 1С или складской программой, когда один и тот же товар продаётся и онлайн, и в зале.',
      'Четвёртая причина — скорость, и её обычно недооценивают. Магазин на коробке тянет за собой чужие скрипты и плагины; на мобильном интернете это лишние секунды до первой отрисовки, а каждая секунда на карточке товара стоит части заказов. В нашем кейсе Smart Fashion страница открывается за 0.8 секунды, и конверсия после редизайна выросла на 65% — это не про красоту, это про то, сколько людей дожидаются загрузки.',
      'И пятое — данные. На своей платформе база клиентов и заказов лежит в вашем аккаунте, а не в чужом кабинете с экспортом по расписанию. Если через два года вы решите сменить подрядчика, вы отдаёте код и дамп базы, а не просите платформу отдать вам ваших же покупателей.',
    ],
    scope: {
    ...
    notFor: {
      ...
      items: [
        ...
        },
        {
          ...
          text: 'То, что делается по замерам, под проект или по договорной цене, в корзину не кладут. Каталог и форма запроса здесь работают лучше checkout.',
        },
        {
        ...
        },
        {
          ...
          text: 'Свой магазин снижает комиссию, но трафик придётся приводить самим. Если маркетплейс работает, а бюджета на рекламу нет, свой канал может быть преждевременным.',
        },
      ],
      ...
      items: [
        {
          ...
          a: 'Для стандартного каталога и стандартного checkout — да, и мы говорим это прямо. Разница проявляется там, где ваши варианты товара, правила скидок или синхронизация со складом не влезают в модель коробки. На брифе считаем оба варианта.',
        },
        {
        ...
        },
        {
          ...
          a: 'Обычно вы, через админку. Если есть существующая база — Excel, 1С, старый сайт — перенос делаем мы: это отдельный этап и он виден в смете.',
        },
        {
